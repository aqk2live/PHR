...
public class UploadFileOrg extends HttpServlet {

    String saveFile ="C:\\Users\\Abdulquader\\Documents\\NetBeansProjects\\PHR\\web\\files";
    
    String uname,docid,itemname,org_id,description;
    HttpSession session;
            ...
            if(!ismultipart){
              ...
              while(itr.hasNext()){
                  ...
                  if(item.isFormField()){
                       ...
                       org_id=OrgSignInServ.orgid;
                       DBConnect dbc = new DBConnect();
            
                        Statement st = dbc.dbCon();
                        
                        String sql = "INSERT INTO orgdocs("+"doc_id,"+"doc_name,"+"orgid,"+"description)"+"values"
                    +                  "('"+docid+"','"+itemname+"','"+org_id+"','"+description+"') ;";
                        
                        int rows_inserted=st.executeUpdate(sql);
                        
                        if(rows_inserted>0){
                        
                             RequestDispatcher req=request.getRequestDispatcher("OrgDocumentUploadSuccess.jsp"); 
   ...
   //  want to prepend orgname to file name
