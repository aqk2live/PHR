<%-- 
    Document   : ShareDocumentwithOrgs
    Created on : 23-Oct-2014, 7:34:00 AM
    Author     : Abdulquader
--%>
...
<div class="main">
        ...
        <div class="article">
          ...
          <% 
            ...
            String sql = "select * from orgs ;";
             ...
             Statement st1 = dbc.dbCon();
             
             String sql1 = "select * from phrownerdocs where patientid='"+PHROwnerSignInServ.patient_id+"' ;";
          
             ResultSet rs1 = st1.executeQuery(sql1);
          %> 
          <form action="ShareDocumentwithOrgServ" method="post">
  ...
  <% while(rs1.next()) {%>
  <option value="<%=rs1.getString("doc_name")%>"><%=rs1.getString("doc_name")%></option>
  <% }%>
</select></td>   
  </tr>
      <td><h3>Select Organization :</h3></td>
      <td><select name="name">
              <option value="Select Organization">Select Organization</option>
  
  <% while(rs.next()) {%>
  <option value="<%=rs.getString("orgname")%>"><%=rs.getString("orgname")%></option>
  <% }%>
</select></td>   
  </tr>
<tr>
